@extends((request('readonly') || request('iframe') || request('fullscreen')) ? 'layouts.plain' : 'layouts.customer')
...
<div class="flex items-center gap-4">
    <a href="{{ $backUrl ?? route('customer.dashboard') }}" class="text-slate-400 hover:text-white transition flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-slate-700" title="Back">
...
<nav class="absolute top-0 left-0 right-0 z-50 p-4 pointer-events-none">
    <div class="pointer-events-auto inline-flex items-center gap-4 bg-slate-900/80 backdrop-blur-md px-4 py-2.5 rounded-2xl border border-white/10 shadow-2xl">
        <a href="{{ $backUrl ?? route('customer.dashboard') }}" class="w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition">
            <i class="fas fa-arrow-left text-sm"></i>
        </a>
        <div class="h-6 w-px bg-white/10"></div>
        <div>
            <h1 class="text-xs font-bold text-slate-400 uppercase tracking-wider">3D Visualizer</h1>
...
<script type="module">
    ...
    // Pass order_id if present in the current request URL (for viewing specific quotes)
    const API_FETCH = "{!! request()->routeIs('guest.*') ? URL::signedRoute('guest.vehicle.inspection.data', ['id' => $vehicle->id, 'order_id' => request('order_id')]) : route('customer.vehicle.inspection.data', ['id' => $vehicle->id, 'order_id' => request('order_id')]) !!}";
    ...
    function renderDefects() {
        ...
        defectListEl.innerHTML = defects.map((d, i) => {
            ...
            <div onclick="focusDefect(${i})" class="group relative bg-white border border-slate-100 rounded-xl p-4 shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer ${theme.border} ${theme.bg}">
                ...
                <div class="space-y-3">
                    <div class="w-full bg-slate-50/50 border border-slate-200 rounded-lg px-3 py-2 text-xs font-semibold text-slate-700 flex items-center gap-2">
                        <i class="fas fa-circle text-[8px] ${theme.text}"></i>
                        ${d.severity === 'critical' ? 'Nghiêm trọng' : (d.severity === 'medium' ? 'Trung bình' : 'Nhẹ')}
                    </div>
                    
                    <div class="w-full bg-slate-50/50 border border-slate-200 rounded-lg px-3 py-2.5 text-xs text-slate-700 min-h-[60px]">
